...
DepthMapPixelHypothesis is the per-pixel state vector for the semi-dense depth map.
Each pixel that “matters” (enough gradient) carries a tiny probabilistic model of its inverse depth plus book-keeping flags.
LSD-SLAM updates these hypotheses over time from direct image alignment, fuses them, prunes them,
and finally hands the “best” (smoothed) values to frames / pose-graph.
